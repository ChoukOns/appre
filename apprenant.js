...
const [show, setShow] = useState(false);
  ...
  useEffect(() => {
    const fetchApprenant = async () => {
      if (apprenantId) {
        console.log('Fetching apprenant with ID:', apprenantId);
        try {
          const response = await axios.get(`http://localhost:3001/api/apprenants/${apprenantId}`);
          setApprenant(response.data);
          console.log('Fetched apprenant:', response.data);
        } catch (error) {
          console.error('Error fetching apprenant:', error.message);
          setError(error.message);
        }
      } else {
        console.warn('apprenantId is not provided');
      }
    };
  ...
  useEffect(() => {
    const fetchCours = async () => {
      if (coursId) {
        console.log('Fetching cours with ID:', coursId);
        try {
          const response = await axios.get(`http://localhost:3001/api/cours/${coursId}`);
          setCours(response.data);
          console.log('Fetched cours:', response.data);
        } catch (error) {
          console.error('Error fetching cours:', error.message);
          setError(error.message);
        }
      } else {
        console.warn('coursId is not provided');
      }
    };
  ...
  const handleConfirm = async () => {
    ...
    console.log('handleConfirm called with apprenantId:', apprenantId, 'and coursId:', coursId);
    
    if (!apprenantId || !coursId) {
      console.log('ID d\'apprenant ou ID de cours non fourni');
    ...
    try {
      const achatData = {
        apprenantId: user?.name, // Utiliser apprenantId ici
        coursId: courses?._id, // Utiliser coursId ici
        montant: courses?.prix, // Utiliser cours ici
        dateA: new Date().toISOString().split('T')[0]
      };

      console.log('Sending achat data:', achatData);
      const response = await axios.post('http://localhost:3001/api/acheter-cours', achatData);

      if (response.status === 200 && response.data.message === "Cours acheté avec succès") {
...
<Modal show={show} onHide={handleClose}>
  ...
  <Modal.Footer>
    {error && <div style={{ flex: 1, color: 'red' }}>{error}</div>}
